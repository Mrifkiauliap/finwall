...
import { CircleAlert, Loader2, Lock, LogIn, Mail, Phone, User } from 'lucide-vue-next'
...
const { defineField, handleSubmit, errors, isSubmitting } = useForm({
  ...
  initialValues: { username: '', email: '', phone: '', password: '', timezone: tz },
})
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    notify({
      ...
      message: t('auth.SIGNUP_SUCCESS_MESSAGE', { username: values.username }),
    })
...
<template>
  <AuthShell :title="t('auth.SIGNUP_TITLE')" :subtitle="t('auth.SIGNUP_SUBTITLE')" :icon="LogIn">
    <div
      v-if="errorMessage"
    ...
    <p class="border-t pt-5 text-center text-sm text-muted-foreground">
      {{ t('auth.HAVE_ACCOUNT') }}
      <RouterLink to="/signin" class="ml-1 font-medium text-primary hover:underline">
        {{ t('auth.SIGNIN_LINK') }}
      </RouterLink>
    </p>
  </AuthShell>
