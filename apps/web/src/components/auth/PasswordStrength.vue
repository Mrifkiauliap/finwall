...
const rules = computed(() => [
  ...
  { key: 'upper', label: t('auth.PWD_RULE_UPPER'), ok: /[A-Z]/.test(model.value) },
  { key: 'lower', label: t('auth.PWD_RULE_LOWER'), ok: /[a-z]/.test(model.value) },
  { key: 'number', label: t('auth.PWD_RULE_NUMBER'), ok: /[0-9]/.test(model.value) },
  { key: 'special', label: t('auth.PWD_RULE_SPECIAL'), ok: /[^a-zA-Z0-9]/.test(model.value) },
])

const passed = computed(() => rules.value.filter((r) => r.ok).length)

const rating = computed(() => (model.value ? passed.value : 0))
...
const hint = computed(() => t('auth.PWD_REQUIREMENTS', { count: passed.value }))
</script>
...
<template>
  <div class="space-y-2" aria-live="polite">
    <div class="flex items-center gap-2">
      ...
      <span v-if="label" class="shrink-0 text-xs font-medium" :class="tone.text">
        {{ label }}
      </span>
    </div>
