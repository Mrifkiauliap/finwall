...
const activeName = computed(
  () => activeTenant.value?.name ?? authStore.currentTenant?.name ?? t('workspace.NO_WORKSPACE'),
)

const activeRole = computed(() => activeTenant.value?.role ?? authStore.currentTenant?.role ?? '-')

const initial = computed(() => (activeName.value || 'F').charAt(0).toUpperCase())
...
/** Muat ulang daftar tenant sambil menampilkan overlay loading. */
...
// dibuka dulu. Pakai watch karena saat hard reload `isAuthenticated` baru
// menjadi true setelah restore sesi selesai.
watch(
...
/**
 * Pindah workspace = pindah URL tenant. Navigasi SPA karena tenant aktif berasal
 * dari URL, bukan dari state session global.
 */
async function selectTenant(tenantId: string) {
  ...
  const targetName = authStore.tenants.find((x) => x.publicId === tenantId)?.name ?? ''
  ...
  try {
    ...
    await router.push(`/t/${tenantId}/dashboard`)
  ...
  <div ref="rootEl" class="relative">
    ...
    >
      ...
      <div v-if="compact" class="min-w-0">
        <p class="truncate text-sm font-semibold leading-tight">{{ activeName }}</p>
      </div>
      <template v-else>
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm font-semibold leading-tight">{{ activeName }}</p>
          <p class="truncate text-xs capitalize text-muted-foreground">{{ activeRole }}</p>
        </div>
      </template>

      <Loader2 v-if="switching" class="size-4 shrink-0 animate-spin text-muted-foreground" />
      ...
      >
        ...
        <template v-else>
          ...
          >
            ...
            <span class="text-xs capitalize text-muted-foreground">{{ tenant.role }}</span>
            <Check v-if="tenant.publicId === tenantPublicId" class="size-4 text-primary" />
          </button>
