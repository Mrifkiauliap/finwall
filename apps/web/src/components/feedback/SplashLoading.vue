...
<template>
  <div class="relative flex items-center justify-center overflow-hidden bg-background">
    <div class="pointer-events-none absolute inset-0" aria-hidden="true">
      <div
        class="absolute left-1/2 top-1/2 size-[42rem] -translate-x-1/2 -translate-y-1/2 animate-splash-breathe rounded-full bg-primary/[0.07] blur-3xl"
      />
      <div class="absolute -left-32 -top-32 size-96 rounded-full bg-primary/10 blur-3xl" />
      <div class="absolute -bottom-32 -right-32 size-96 rounded-full bg-primary/5 blur-3xl" />
      <div
    ...
    <div class="relative z-10 flex flex-col items-center gap-7 px-6 text-center">
      <div class="relative flex size-20 items-center justify-center">
        <span
          class="absolute inset-0 animate-splash-glow rounded-3xl bg-primary/20 blur-xl"
          aria-hidden="true"
        />
        <span
          class="absolute inset-0 animate-splash-spin rounded-[1.35rem] border-2 border-primary/25 border-t-primary"
          aria-hidden="true"
        />
        <div
      ...
      <div class="space-y-2">
        <h1 class="animate-splash-rise text-3xl font-bold tracking-tight text-foreground">
          {{ title ?? t('loading.TITLE') }}
        </h1>
        <p class="animate-splash-rise text-sm text-muted-foreground [animation-delay:120ms]">
      ...
      <div class="w-56 space-y-3">
        <div class="h-1 overflow-hidden rounded-full bg-muted">
          <span class="block h-full w-1/3 animate-splash-bar rounded-full bg-primary" />
        </div>
        <div class="flex justify-center gap-1.5" aria-hidden="true">
          <span
            v-for="i in 3"
            :key="i"
            class="size-1.5 animate-splash-dot rounded-full bg-primary/70"
