...
export interface ResolvedNavGroup extends Omit<NavGroup, 'groupNameKey' | 'items'> {
  groupName?: string
...
export const useNavigation = () => {
  ...
  const filteredNavigation = computed<ResolvedNavGroup[]>(() => {
    return navigationConfig
      .map((group) => ({
        groupName: group.groupNameKey ? t(`common.navigation.${group.groupNameKey}`) : undefined,
  ...
  const currentPath = computed(() => {
    ...
    const stripped =
      prefix && route.path.startsWith(prefix) ? route.path.slice(prefix.length) : route.path
