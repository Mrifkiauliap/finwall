...
const groups = computed(() => filteredNavigation.value)
</script>
      ...
      <template v-for="(group, gIdx) in groups" :key="group.groupName ?? gIdx">
        ...
        >
          <span
            ...
            :class="isActive(item.basePath) ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'"
            aria-hidden="true"
          />
          ...
          />
          <span class="w-full truncate px-0.5 text-center leading-none">{{ item.name }}</span>
        </RouterLink>
      </template>
    </div>
    ...
    <div class="w-full border-t p-2">
      ...
      >
        <Transition name="icon-swap" mode="out-in">
          <PanelLeftOpen v-if="!isPanelOpen" key="open" class="size-[1.15rem]" />
