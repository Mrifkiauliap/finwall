...
const variantStyles = computed(() => {
  return {
    ...
    success: {
      ...
      badge: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 dark:bg-emerald-500/20',
      progress: 'bg-emerald-500',
    },
    warning: {
      card: 'border-amber-500/20 bg-background/95 dark:bg-zinc-900/95 shadow-amber-500/5',
      accent: 'bg-amber-500',
      badge: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 dark:bg-amber-500/20',
      progress: 'bg-amber-500',
    },
    danger: {
      card: 'border-rose-500/20 bg-background/95 dark:bg-zinc-900/95 shadow-rose-500/5',
      accent: 'bg-rose-500',
      badge: 'bg-rose-500/10 text-rose-600 dark:text-rose-400 dark:bg-rose-500/20',
...
<template>
  ...
  >
    <div
      :class="[
        'absolute bottom-0 left-0 top-0 w-1.5 rounded-l-xl opacity-90',
    ...
    <div class="flex w-full items-start gap-3.5 pl-1">
      ...
      <div class="min-w-0 flex-1 pt-0.5">
        <p v-if="notification.title" class="text-sm font-semibold tracking-tight text-foreground">
          {{ notification.title }}
        </p>
        ...
        <p
          :class="[
            ...
            notification.title ? 'mt-0.5' : 'text-sm font-medium text-foreground/90',
          ]"
        >
    ...
    <div
      ...
      class="absolute bottom-0 left-0 right-0 h-0.5 overflow-hidden bg-muted/30"
    >
      <div
        :class="['progress-timer h-full w-full', variantStyles.progress]"
