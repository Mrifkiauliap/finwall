...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    // `notify()` (bukan `notifyAfterReload`): navigasi berikutnya adalah SPA
    // (`router.push`), jadi halaman TIDAK dimuat ulang. `notifyAfterReload`
    // menulis ke sessionStorage yang hanya dibaca sekali saat bootstrap,
    // sehingga toast baru muncul setelah user me-refresh manual.
    // `NotificationContainer` terpasang global di `App.vue` (di luar
    // `RouterView`), jadi toast tetap tampil setelah pindah halaman.
    notify({
      type: 'success',
      title: t('auth.SIGNIN_SUCCESS_TITLE'),
      message: t('auth.SIGNIN_SUCCESS_MESSAGE', { identifier: values.identifier }),
    })
...
<template>
  <AuthShell :title="t('auth.SIGNIN_TITLE')" :subtitle="t('auth.SIGNIN_SUBTITLE')" :icon="LogIn">
    <div
      v-if="errorMessage"
    ...
    <p class="border-t pt-5 text-center text-sm text-muted-foreground">
      {{ t('auth.NO_ACCOUNT') }}
      <RouterLink to="/signup" class="ml-1 font-medium text-primary hover:underline">
        {{ t('auth.SIGNUP_LINK') }}
      </RouterLink>
    </p>
  </AuthShell>
